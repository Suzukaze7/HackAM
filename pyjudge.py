...
PYJUDGE_RUNNER_SPECIAL_TLE = 20.0
...
class Judger:
    ...
    def specialJudge(self, spj: list, inp: Path, outp: Path) -> Result:
        ...
        res = Runner(60).specialJudgeRun(
            spj, in_out, FileCtrl.workPath("verdict.txt"))
        return res
...
class Tester:
    ...
    def hack(
        ...
        if st.gen_err:
            log.warning(Ansi.bold_red(
                "Generator Error:" + " " + st.res.format()))
        elif st.std_err:
            log.warning(
                Ansi.bold_red("Standard Solution Error:" +
                              " " + st.res.format())
            )
        elif st.spj_err:
            log.warning(Ansi.bold_red(
                "Special Judge Error:" + " " + st.res.format()))
        else:
        ...
        if st.gen_err or st.std_err or st.spj_err:
            ...
            subprocess.run(
                [r'C:\Users\21396\AppData\Local\Programs\Microsoft VS Code\bin\code.cmd', in_target])
...
def main():
    ...
    parser.add_argument("--py-interpreter",
                        help="specify the interpreter for Python")
    ...
    exgroup.add_argument("-s", "--std", type=Path,
                         help="standard solution code")
    ...
    def prepare(file: Path, name: str) -> list:
        if not file.exists():
            log.error(file.name + " " +
                      Verdict.NO_SUCH_FILE_OR_DIRECTORY.format())
            return
